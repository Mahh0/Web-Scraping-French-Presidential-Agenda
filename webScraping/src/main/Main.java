...
public class Main {
	...
	// Définition d'un loggeur pour afficher les erreurs en console ou en fichier de
	// logs.
	public static void main(String[] args) throws Exception {
		logger.info(
				"Starting the program. \n Note : Firewall must be set to OFF if you wan't to use a remote SQL Server !");

		MySqlConnection.setParameters();
		Connection con = new MySqlConnection().getConnection();
		/**
		 * Call of the setParameters method from MySqlConnection class which set
		 * database parameters and others, using config.properties file.
		 * 
		 * Instantiating a connection (con) to MySql Database using getConnection method
		 * from MySqlConnection class
		 * This connection is used in the classes in which we need interactions with
		 * database. It uses ReadPropertyFile class.
		 */

		switch (ReadPropertyFile.getDatabasecleanup()) {
			case "true":
				logger.info(
						"Value \"true\" found in properties file for the parameter databaseCleanup. Cleaning database...");
				new ClearTablesAtProgramStartup();
				break;
			case "false":
				logger.info(
						"Value \"false\" found in properties file for the parameter databaseCleanup. Nothing happening...");
				break;
			default:
		...
		/**
		 * This asks through the parameter in the config file to the user if he want's to
		 * clear the tables. If yes, it is cleaned, if no, no, if not yes or no, exit
		 * program (error)
		 */
		...
		/**
		 * Reading "LocalfilesCleanup parameter from config file. If it is set to true, files will
		 * not be kept (=deleted), if false they are keep, else => parameter error
		 */
		...
		/**
		 * Getting current month and year (this will be useful, particulary to not
		 * download the current
		 * month page, because it is in progress so if there is content being added we
		 * will not get it with local files)
		 */

		MainLinks links = new MainLinks();
		/**
		 * Instantiating the MainLinks class which generate links to analyse. This
		 * returns an ArrayList containing the generated links.
		 */
		...
		for (Object link : links.AnalyseLiens()) {
			...
			/**
			 * Basic things :
			 * Getting the month and the year (we can also get it with MainLinks but it's
			 * easier like that), thanks to splitters
			...
			switch (ReadPropertyFile.getUseLocalHtml()) {
				case "yes": // We wan't to use local html files if possible
					File input = new File(
							"webScraping/src/main/resources/htmlAgenda/html-" + month + "-" + year + ".html");
					if (input.exists() == true) {
						doc = Jsoup.parse(input, "UTF-8");
					} else {
						doc = SSLHelper.getConnection(link.toString())
								.userAgent("Mozilla/5.0 (Windows NT 6.1; WOW64; rv:5.0) Gecko/20100101 Firefox/5.0")
								.timeout(50000).get();
						if (link.toString() != "http://www.elysee.fr/agenda-" + cmonthWaccent + "-" + cyear) { // while
																												// the
																												// link
																												// is
																												// not
			 ...
			 * 
			 * If in properties files we wan't to use Local HTML for agenda (yes)
			 * => we read the file, and if it exists, we put it in a doc and continue the
			 * program
			 * => else (if it doesn't exists), we make a GET request to the website, we add
			 * it to local files and we put it into a doc
			 * 
			...
			for (Element container : containers) {
				/**
				 * For each container (day) in containers (days)
				 */
				 ...
				 * 	- day : it is only the day (numeric) like 06, selected in the h3 sticker
				...
				for (Element row : rows) {
					/**
					 * Now we are analyzing rows, containing for each row an event, with hour, type,
					 * entitled and 0, 1 or several links.
					 */
					String eventhour = row.select("div.list-table__details > p.list-table__hour").text(); // We select
																											// the hour
																											// of the
																											// event (ex
																											// : 09:30,
																											// Morning,
																											// Afternoon)
					String eventtype = row.select("div.list-table__details > p.list-table__type").text(); // We select
																											// the type
																											// of the
																											// event (ex
																											// :
																											// Meeting)
					String entitled = row.select("div.list-table__content > p.m-b-n").first().text(); // We select the
																										// entitled
					Elements externallinkslist = row.select("a[href]"); // We select the links (ressources) for this
																		// event => there can be several so stocked in
																		// Elements
					/**
					 * Selecting the eventhour, the eventtype, the entitled, and we are creating a
					 * list of elements (externallistlinks) to analyse them.
					 */
					...
					/**
					 * Sometimes, the hour can be "Matin", "Après-Midi" or "Toute la journée)", so
					 * we define a numeric hour
					 */

					String eventstartdate = year + "-" + monthFormat + "-" + day + " " + eventhour + ":00";
					/**
					 * At this step, we have enought for the event start date : year, month, day and
					 * hour.
					 */
					...
					/**
					 * Insert of an event with the start date, the type and the entitled. We just
					 * need to add the duration now (and type enrichment when needed)
					 */
					...
					/**
					 * Recovery of the ID of the current  current, and the previous event ID inserted in
					 * database.
					 * Theses will be needed for events duration (operations between 2 events)
					 */

					DataEnrichment te = new DataEnrichment();
					// Instantiating dataEnrichment class (for personnalities and types)

					if (eventtype.isEmpty()) {
						te.typeEnrich(entitled, eventtype, currentEventID);
					}
					// Calling TypeEnrich method to try to add a type to the event, only if the eventtype is empty.

					te.personnalitesEnrich(entitled, currentEventID);
					// Calling personnalitesEnrich method to try to guess if there are peoples involved in the event.


					PreparedStatement LastEventDate_ps = con
							.prepareStatement("SELECT dated, type FROM evenement WHERE id= (?)");
					...
					to.close();
					/**
					 * Selecting start date and type from the preivous event for durations
					 * calculations
					 */
					...
					PreparedStatement pSduree = con.prepareStatement("UPDATE evenement SET duree = (?) WHERE id= (?) "); // Ce
																															// statement
																															// et
																															// ce
																															// qui
					...
					pSduree.close();
					/**
					 * Calling EventsTimeManagement method, in a String (timeformat) to calculate
					 * length of the preivous event.
					 */
					...
					/**
					 * Now that we did everything related to the "agenda", we can now analyse
					 * external links. Everything is done in this method.
					 */
				}
			}
		}
